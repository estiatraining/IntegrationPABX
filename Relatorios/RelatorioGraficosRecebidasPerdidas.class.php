...
class RelatorioGraficosRecebidasPerdidas extends Utilitarios implements RelatorioNegociosInterface {
    public function __construct(){}
    public function gerarGraficosRecebidasPerdidas($_dateIni, $_dateFim, $_timeIni , $_timeFim , $_ramal){
        if(file_exists($_SERVER[ 'DOCUMENT_ROOT' ]."/System/IMG/grafico01.png"))
            unlink($_SERVER[ 'DOCUMENT_ROOT' ]."/System/IMG/grafico01.png");
        ...
        $PHPlot->SetTitle( utf8_decode("Ligações Atendidas e Não Atendidas por hora." ));
        ...
        $PHPlot->SetLegend(array('Recebidas', 'Perdidas'));//inserido a descrição na legenda do grafico
        ...
        $quantidade = array();
        $quantidade1 = array();
        while ($row = $DadosFiltradosDao->resultObject($result['totalLigacoesRecebidasHora'])) {
            $quantidade[$row->hora] = $row->quantidade;
        }
        $result = $DadosFiltradosDao->chamadasAtendidasPerdidasHora($values);
        while ($row = $DadosFiltradosDao->resultObject($result['totalLigacoesPerdidasHora'])) {
            $quantidade1[$row->hora] = $row->quantidade;
        }
        $dadosArray = array(
        ...
        $PHPlot->output_file = $_SERVER[ 'DOCUMENT_ROOT' ]."/System/IMG/grafico01.png";
